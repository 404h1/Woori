...
export default function Page06_FundJoin({ onBack, onNext }) {
  ...
  return (
    <div className="phone-frame">
      ...
      <div className="scroll-content" style={{ padding: '24px 20px 100px' }}>
        ...
        {step === 3 ? (
          <>
            ...
            {/* 가입금액 */}
            <div style={{ border: '1.5px solid #e5e7eb', borderRadius: 12, padding: '16px', marginBottom: 4 }}>
              <div style={{ fontSize: 12, color: '#aaa', marginBottom: 8 }}>가입금액</div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4 }}>
                <input
                  ...
                  style={{ border: 'none', outline: 'none', fontSize: 18, color: '#111', textAlign: 'right', width: '100%', background: 'transparent' }}
                />
                <span style={{ fontSize: 18, color: '#111', fontWeight: 600 }}>원</span>
              </div>
            </div>
            <div style={{ textAlign: 'right', fontSize: 13, color: '#888', marginBottom: 28 }}>0원</div>
            ...
            <div style={{ background: '#eef4ff', borderRadius: 12, padding: '16px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: '#1b64da' }}>0원</span>
              </div>
      ...
      <div style={{ padding: '12px 20px 24px', background: '#fff', flexShrink: 0 }}>
        <button
          ...
          onClick={handleNext}
          disabled={step === 3 && !amount}
          style={step === 3 && !amount ? { background: '#c0cfe8' } : {}}
        >
          {step === 3 ? '다음' : '가입하기'}
        </button>
      </div>
