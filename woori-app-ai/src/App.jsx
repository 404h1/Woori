...
// 화면 순서
// 01(설문) → 02(결과) → 03(목록) → 03b(상세) → 03c(AI설명) 또는 04(비교) → 05(투자자확인) → 06(가입) → 07(설명서) → 08(해피콜)

const PAGE_ORDER = ['01','02','03','03b','03c','04','05','06','07','08'];
...
export default function App() {
  ...
  const go = (p, dir) => {
    ...
    const to   = PAGE_ORDER.indexOf(p);
    setAnimDir(dir ?? (to >= from ? 'forward' : 'back'));
    setAnimKey(k => k + 1);
    setPage(p);
  };

  const cls = animDir === 'forward' ? 'page-enter-forward' : 'page-enter-back';

  return (
    <div key={animKey} className={cls}>
      {page === '01' && <Page01 onNext={() => go('02')} />}
      {page === '02' && <Page02 onNext={() => go('03')} onBack={() => go('01', 'back')} />}
      {page === '03' && (
        <Page03
          onNext={() => go('04')}
          onFundDetail={() => go('03b')}
