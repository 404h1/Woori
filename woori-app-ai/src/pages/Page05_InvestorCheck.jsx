...
const SCRIPT = '투자 상품 가입 전에 몇 가지 확인이 필요해요. 대출과 연계된 투자를 권유받으셨는지, 그리고 전문금융소비자에 해당하시는지 확인해 주세요. 아래 질문에 하나씩 답해 주시면 돼요.';
    ...
    <div className="phone-frame">
      ...
      <div className="scroll-content" style={{ padding: '20px 20px 24px' }}>
        ...
        <div style={{ background: '#f8fafc', borderRadius: 14, padding: '16px', marginBottom: 20 }}>
          ...
          {step === 0 && (
            <>
              {['아니요', '예'].map(label => (
                <button key={label} onClick={() => { setAns1(label); setStep(1); }}
                  style={{ width: '100%', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: '14px 16px', fontSize: 15, color: '#222', cursor: 'pointer', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {step >= 1 && (
            <>
              {['아니요', '예'].map(label => (
                <div key={label} style={{ width: '100%', background: '#f0f2f5', border: '1px solid #e5e7eb', borderRadius: 10, padding: '14px 16px', fontSize: 15, color: '#aaa', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ background: '#f8fafc', borderRadius: '4px 16px 16px 16px', padding: '16px' }}>
                  ...
                  {step === 1 && (
                    <>
                      {['아니오', '예'].map(label => (
                        <button key={label} onClick={() => { setAns2(label); setStep(2); }}
                          style={{ width: '100%', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: '14px 16px', fontSize: 15, color: '#222', cursor: 'pointer', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          {label} <span style={{ color: '#ccc' }}>✓</span>
                        </button>
                      ))}
                  ...
                  {step >= 2 && (
                    <>
                      {['아니오', '예'].map(label => (
                        <div key={label} style={{ background: '#f0f2f5', border: '1px solid #e5e7eb', borderRadius: 10, padding: '14px 16px', fontSize: 15, color: '#aaa', marginBottom: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div className="animate-chat-appear" style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
              ...
              <div style={{ flex: 1, background: '#f8fafc', borderRadius: '4px 16px 16px 16px', padding: '16px' }}>
                ...
                <div style={{ marginBottom: 8 }}>
                  ...
                  {[
                    ['이 금융상품은 ', '예금자보호법에 따라 보호되지 않습니다.', '#1b64da'],
                    ['운용실적에 따라 ', '원금손실이 발생할 수 있으며, 손실가능범위는 각 상품에 따라 다를 수 있습니다.', '#dc2626'],
                    ['환매 또는 해지시 환매수수료가 발생할 수 있습니다. 또한 집합투자규약 및 투자설명서에 따라 중도인출이 불가할 수 있습니다.', '', ''],
                  ].map(([t1, t2, c2], i) => (
                    <div key={i} style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                      <span style={{ color: '#999', flexShrink: 0 }}>•</span>
                      <p style={{ fontSize: 13, color: '#444', lineHeight: 1.6 }}>
                        {t1}{t2 && <span style={{ color: c2, fontWeight: 600 }}>{t2}</span>}
                      </p>
                    </div>
                  ))}
                </div>

                <button onClick={onClose} style={{ width: '100%', background: '#f2f4f6', border: '1px solid #e5e7eb', borderRadius: 10, padding: '14px 16px', fontSize: 15, color: '#555', cursor: 'pointer', marginTop: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      {showVoice && (
        <VoiceGuide
          ...
          onCommand={(cmd) => {
            if (cmd.includes('확인') && step >= 2) onClose();
            else if (cmd.includes('아니') && step === 0) { setAns1('아니요'); setStep(1); }
            else if (cmd.includes('예') && step === 0) { setAns1('예'); setStep(1); }
          }}
        />
      )}
    </div>
