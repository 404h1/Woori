...
const SCRIPT = '수익률 BEST 펀드 목록이에요. 고객님 투자 성향에 맞는 상품들이에요. 원하는 펀드를 눌러 상세 내용을 볼 수 있고, 두 개를 선택하면 비교도 가능해요. 삼성 글로벌 반도체 펀드가 최근 3개월 수익률이 46%로 가장 높아요.';
...
export default function Page03_FundList({ onNext, onFundDetail, onBack }) {
  ...
  return (
    <div className="phone-frame">
      ...
      {selected.length > 0 && (
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #f0f2f5', padding: '12px 16px 24px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <span style={{ fontSize: 14, fontWeight: 700 }}>선택된 상품 <span style={{ color: '#1b64da' }}>{selected.length}개</span></span>
            {selected.length === 2 && (
              <button onClick={onNext} style={{ background: '#1b64da', color: '#fff', border: 'none', borderRadius: 20, padding: '8px 20px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>비교하기</button>
            )}
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            {selected.map(f => (
              <div key={f.id} style={{ background: '#f2f4f6', borderRadius: 20, padding: '6px 12px', fontSize: 12, color: '#333', display: 'flex', alignItems: 'center', gap: 6, maxWidth: '45%', overflow: 'hidden' }}>
                <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{f.name.slice(0, 10)}...</span>
                <button onClick={() => toggleSelect(f)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#888', fontSize: 14, flexShrink: 0 }}>×</button>
              </div>
            ))}
            {selected.length < 2 && (
              <div style={{ background: '#f2f4f6', borderRadius: 20, padding: '6px 16px', fontSize: 12, color: '#aaa' }}>추가하기</div>
            )}
          </div>
        </div>
      )}
      ...
      {showVoice && (
        <VoiceGuide
          ...
          onCommand={(cmd) => {
            if (cmd.includes('비교')) onNext();
            else if (cmd.includes('상세') || cmd.includes('반도체')) onFundDetail?.();
