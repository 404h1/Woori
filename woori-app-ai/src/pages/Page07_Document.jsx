...
const SCRIPT = '집합투자상품 설명서예요. 투자에 따른 위험과 원금 손실 가능성 등 중요한 내용이 담겨 있으니 끝까지 읽어 주세요. 다 읽으신 후 직접 터치해서 확인 버튼을 눌러 주셔야 해요.';
...
export default function Page07_Document({ onClose, onNext }) {
  ...
  return (
    <div className="phone-frame">
      ...
      <div ref={scrollRef} onScroll={handleScroll} className="scroll-content" style={{ padding: '20px 20px 100px' }}>
      ...
      <div style={{ padding: '12px 20px 24px', background: '#fff', flexShrink: 0 }}>
        {!scrolled && (
          <div style={{ background: '#333', color: '#fff', borderRadius: 10, padding: '12px 16px', fontSize: 13, textAlign: 'center', marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
            ⚠ 내용을 끝까지 확인해 주세요.
          </div>
        )}
        {scrolled && (
          <div className="direct-touch-badge" style={{ marginBottom: 10 }}>
            ☝ 직접 터치해서 확인해 주세요
          </div>
        )}
      ...
      {showVoice && (
        <VoiceGuide
          ...
          onCommand={(cmd) => {
            if (cmd.includes('확인') && scrolled) onNext();
          }}
        />
      )}
    </div>
