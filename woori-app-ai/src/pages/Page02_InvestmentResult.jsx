...
const SCRIPT = '이혜원 고객님, 투자성향 분석 결과가 나왔어요. 고객님의 성향은 공격투자형입니다. 주식이나 주식형 펀드처럼 위험은 높지만 기대 수익도 높은 상품에 적합한 유형이에요. 아래 확인 버튼을 눌러 맞춤 펀드를 확인해 보세요.';
  ...
  return (
    <div className="phone-frame">
      ...
      <div className="scroll-content" style={{ padding: '28px 20px 100px' }}>
        ...
        {/* 투자성향 차트 */}
        <div style={{ background: '#f8fafc', borderRadius: 16, padding: '20px 16px 16px', marginBottom: 32 }}>
          <div style={{ fontSize: 12, color: '#888', marginBottom: 16 }}>↑ 기대수익률</div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', marginBottom: 12, height: 96 }}>
            {types.map((t, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 11, fontWeight: i === 4 ? 700 : 400, color: i === 4 ? '#fff' : '#555', whiteSpace: 'pre-wrap', textAlign: 'center', lineHeight: 1.3, flexShrink: 0 }}>
                  {t}
                </div>
              </div>
            ))}
        ...
        {/* 안내 박스 */}
        <div style={{ background: '#f8fafc', borderRadius: 12, padding: '16px', border: '1px solid #e5e7eb', marginBottom: 20 }}>
