...
const SCRIPT_STEP3 = '가입 정보를 입력해 주세요. 출금 계좌를 확인하시고, 아래 금액 입력란을 눌러 가입 금액을 입력해 주세요. 최소 만 원 이상 입력하셔야 해요.';
const SCRIPT_STEP6 = '입력하신 가입 정보를 확인해 주세요. 매수예약금액, 매수예정일, 출금 계좌번호를 확인하신 후 가입하기 버튼을 눌러 주세요.';
...
export default function Page06_FundJoin({ onBack, onNext }) {
  ...
  const script = step === 3 ? SCRIPT_STEP3 : SCRIPT_STEP6;

  const handleNext = () => {
    if (step === 3) { setShowVoice(true); setStep(6); }
    else onNext();
  };
  ...
  return (
    <div className="phone-frame">
          ...
          <>
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 2 }}>
                <input type="text" readOnly value={amount ? Number(amount).toLocaleString() : ''} placeholder="10,000원 이상"
      ...
      {/* 커스텀 키패드 */}
      {showKeypad && step === 3 && (
        <div className="animate-chat-appear" style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: '#fff', borderTop: '1px solid #ebecef', zIndex: 300, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', gap: 8, padding: '12px 16px', background: '#f8fafc', borderBottom: '1px solid #ebecef' }}>
            {['+1만', '+10만', '+100만', '전액'].map(btn => (
              <button key={btn} onClick={() => handleQuickAdd(btn)}
                style={{ flex: 1, padding: '8px 0', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 20, fontSize: 13, fontWeight: 600, color: '#333', cursor: 'pointer' }}>
                {btn}
              </button>
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)' }}>
            {['1','2','3','4','5','6','7','8','9','00','0','⌫'].map(key => (
              <button key={key} onClick={() => handleKeyPress(key)}
                style={{ padding: '20px 0', fontSize: 24, fontWeight: 500, color: '#111', background: 'none', border: 'none', cursor: 'pointer', borderRight: '1px solid #f4f4f4', borderBottom: '1px solid #f4f4f4' }}>
                {key}
              </button>
            ))}
          </div>
          <div onClick={() => setShowKeypad(false)} style={{ padding: '12px', textAlign: 'center', background: '#f8fafc', color: '#888', fontSize: 20, cursor: 'pointer', borderTop: '1px solid #ebecef' }}>∨</div>
        </div>
      )}
      ...
      {showVoice && !showKeypad && (
        <VoiceGuide
          ...
          onCommand={(cmd) => {
            if (cmd.includes('다음') && step === 3 && amount && parseInt(amount, 10) >= 10000) handleNext();
            else if (cmd.includes('가입') && step === 6) onNext();
          }}
        />
      )}
